...
export const AccessPollsCard = ({ product, ...rest }) => {
  ...
  const handleUpdateRoom = async () => {
    ...
    if (revokeEvAccess) {
      await axios
        .put(
          process.env.NEXT_PUBLIC_BASE_URL + "revoke-polls-access?email=" + dialogObj.email,
          update,
          {
            headers: {
      ...
      await axios
        .put(
          process.env.NEXT_PUBLIC_BASE_URL + "give-poll-access?email=" + dialogObj.email,
          update,
          {
            headers: {
              authorization: user.accessToken,
                ...
                {userArray.length > 0
                  ? userArray.slice(0, limit).map((el) => (
                      ...
                      >
                        ...
                        <TableCell>{el.pollsAccess ? ` ${el.pollsAccess}` : "N/A"}</TableCell>
                        <TableCell>
